'use client'; // This is a Client Component that runs in the browser

import { useState, useEffect } from 'react';
import { supabase } from '../../lib/supabaseClient'; // Our existing Supabase connection
import { User } from '@supabase/supabase-js'; // Import User type from Supabase

export default function AuthButton() {
  // Store the user information with proper typing
  const [user, setUser] = useState<User | null>(null);
  // Track loading state
  const [loading, setLoading] = useState(true);

  // Check for existing session when component loads
  useEffect(() => {
    // Get current session
    supabase.auth.getSession().then(({ data }) => {
      setUser(data.session?.user || null);
      setLoading(false);
    });

    // Listen for auth changes
    const { data } = supabase.auth.onAuthStateChange((event, session) => {
      setUser(session?.user || null);
    });

    // Clean up listener when component unmounts
    return () => data.subscription.unsubscribe();
  }, []);

  // Handle GitHub login when button is clicked
  const handleLogin = async () => {
    await supabase.auth.signInWithOAuth({
      provider: 'github',
      options: {
        // No need for complex redirects - Supabase handles this automatically!
        redirectTo: window.location.origin
      }
    });
  };

  // Handle logout when button is clicked
  const handleLogout = async () => {
    await supabase.auth.signOut();
  };

  // Show loading state
  if (loading) {
    return <button className="px-4 py-2 rounded bg-gray-200">Loading...</button>;
  }

  // If user is logged in, show profile and logout button
  if (user) {
    return (
      <div className="flex items-center gap-2">
        {/* Show user's avatar if available */}
        {user.user_metadata?.avatar_url && (
          <img src={user.user_metadata.avatar_url} alt="Profile" className="w-8 h-8 rounded-full" />
        )}
        
        {/* Show user's name */}
        <span>{user.user_metadata?.name || user.email}</span>
        
        {/* Logout button */}
        <button 
          onClick={handleLogout}
          className="ml-2 px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600"
        >
          Logout
        </button>
      </div>
    );
  }

  // If not logged in, show login button
  return (
    <button
      onClick={handleLogin}
      className="px-4 py-2 bg-gray-800 text-white rounded flex items-center gap-2 hover:bg-gray-700"
    >
      {/* GitHub icon */}
      <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
        <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.387.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.387-1.333-1.756-1.333-1.756-1.09-.745.083-.73.083-.73 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.807 1.305 3.492.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12" />
      </svg>
      Login with GitHub
    </button>
  );
}
